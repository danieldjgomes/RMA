...
public class DiferentialBotControl : MonoBehaviour
{
    ...
    float FowardSpeed()
    {
        ...
        float speed = 0;
        int layerMask = 1 << 8;
        Debug.DrawRay(sensorE.transform.position, -sensorE.transform.forward* 0.5f, Color.blue,0.1f,true);
        Debug.DrawRay(sensorD.transform.position, -sensorD.transform.forward * 0.5f, Color.green, 0.1f,true);
        RaycastHit hit;
        if (Physics.Raycast(sensorE.transform.position, -sensorE.transform.forward, out hit, 0.5f, layerMask) || (Physics.Raycast(sensorD.transform.position, -sensorD.transform.forward, out hit, 0.5f, layerMask)))
        {
            textE.text = "O sensor detectou um obstaculo";
            return speed;
        }
        else
        {
